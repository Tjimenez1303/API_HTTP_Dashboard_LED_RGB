...
static esp_err_t form_post_dashboard_handler(httpd_req_t *req) {
    ...
    if (threshold && red_val && green_val && blue_val)
    {
        // Aquí puedes utilizar los valores extraídos del JSON según sea necesario
        // ...
		...
		// app_dashboard_set_threshold(threshold->valuestring);
		// app_dashboard_set_red_val(red_val->valuestring);
		// app_dashboard_set_green_val(green_val->valuestring);
		// app_dashboard_set_blue_val(blue_val->valuestring);
...
static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req){
	...
	float prom_temp = Get_prom_temp();

	sprintf(dhtSensorJSON, "{\"temp\":\"%.1f\",\"humidity\":\"%.1f\"}", prom_temp, 40.5);
